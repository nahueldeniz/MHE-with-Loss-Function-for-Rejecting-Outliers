...
function S = build_setup(S)
    ...
    S.config.CtrlMethod     = 'proposed';   % 'proposed', 'fnmppc'
    S.config.integrator     = 'RK4';        % euler
    S.config.EstMethod      = 'standard';   % 'proposed', 'bayesian', 'standard';
    ...
    S.config.noise_lvl      = [(1*pi/180).*ones(S.config.Nt, 1); 0.2*pi/180; [0.025; 0.025]];%; [0.05; 0.05]]; % measurement noise amplitude: [betas'; theta_0; x_0; y_0; w0; v0]
    ...
    S.config.gpsOutlBurst   = [];%ceil(S.config.Ne*0.5); % 0: no burst, 1 to Ne: number of consecutive outliers
    S.config.gpsOutlBurstCnt = 0; % 0: no burst, 1 to Ne number of consecutive outliers
    S.config.AMPLITUDES     = [1 2 4 8 16 32];
    ...
    S.config.DENSITIES      = [1 5 10 25 50 75];
...
function S = call_init_functions(S,outlDensity,outlAmplitude)
    ...
    % S.config.gpsOutlBurst       = ceil(S.config.Ne*(outlDensity/100));
    % temporarily here:
    if S.config.lidarOn
...
function S = update_measurements(S)
    ...
    if S.config.SIM == true
        ...
        if S.config.gpsOutlier == true
            if S.config.gpsOutlBurst
                if S.config.gpsOutlBurstCnt<=S.config.gpsOutlBurst
                    noise(S.config.Nt+2:S.config.Nt+3) = S.config.gpsOutlierAmp*randn(2,1);
                end
                S.config.gpsOutlBurstCnt = S.config.gpsOutlBurstCnt+1;
                if S.config.gpsOutlBurstCnt == S.config.Ne+1
                    S.config.gpsOutlBurstCnt = 1;
                end
            else
                if randi(100) <= S.config.gpsOutlierProbab
                    if strcmp(S.config.noise,'gaussian')
                        ...
                        noise(S.config.Nt+2:S.config.Nt+3) = S.config.gpsOutlierAmp*(2.*rand(2,1)-1);
                    end
                end
            end            
        end
...
function S = update_EstimatorMeasurement(S)
    if S.config.mhe
        if strcmp(S.config.EstMethod,'proposed')
