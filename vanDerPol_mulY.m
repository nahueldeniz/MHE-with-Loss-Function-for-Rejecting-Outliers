...
function sim = build_setup(sim)
    ...
    %
    sim.config.numTrials            = 10;
    sim.config.numBatchs            = 1;
    if sim.config.numTrials > 1
        sim.config.same_seed = false;
    end
    %
    sim.config.numSensors           = 2;
    %
    sim.config.fracTs               = 10;                        % percentual value: [0, 99]
    sim.config.fractions_of_Ts      = 10:10:90;
    sim.config.delayY               = true;
    %    
end

function sim = set_fraction_of_Ts(sim, fraction)
    sim.config.fracTs = fraction;
end
...
function sim = gen_dynamic(sim)
    ...
    %
    k1          = sim.dynamic.f(sim.dynamic.x,sim.dynamic.u);
    k2          = sim.dynamic.f(sim.dynamic.x + ((sim.config.Ts * (100-sim.config.fracTs)/100)) / 2 * k1, sim.dynamic.u);
    k3          = sim.dynamic.f(sim.dynamic.x + ((sim.config.Ts * (100-sim.config.fracTs)/100)) / 2 * k2, sim.dynamic.u);
    k4          = sim.dynamic.f(sim.dynamic.x + ((sim.config.Ts * (100-sim.config.fracTs)/100)) * k3, sim.dynamic.u);
    x_rk4_MulYDelay   = sim.dynamic.x + ((sim.config.Ts * (100-sim.config.fracTs)/100)) / 6 * (k1 + 2 * k2 + 2 * k3 + k4);
    sim.dynamic.FMulYDelay = casadi.Function('FMulYDelay', {sim.dynamic.x, sim.dynamic.u}, {x_rk4_MulYDelay});
    %
    k1          = sim.dynamic.f(sim.dynamic.x,sim.dynamic.u);
    k2          = sim.dynamic.f(sim.dynamic.x + ((sim.config.Ts * sim.config.fracTs/100)/(sim.config.numSensors-1)) / 2 * k1, sim.dynamic.u);
    k3          = sim.dynamic.f(sim.dynamic.x + ((sim.config.Ts * sim.config.fracTs/100)/(sim.config.numSensors-1)) / 2 * k2, sim.dynamic.u);
    k4          = sim.dynamic.f(sim.dynamic.x + ((sim.config.Ts * sim.config.fracTs/100)/(sim.config.numSensors-1)) * k3, sim.dynamic.u);
    x_rk4_MulY  = sim.dynamic.x + ((sim.config.Ts * sim.config.fracTs/100)/(sim.config.numSensors-1)) / 6 * (k1 + 2 * k2 + 2 * k3 + k4);
...
function sim = build_nlmheAlessandri(sim)
    ...
    sim.mheAle.beta    = 1;
    % *************************************************************
    sim.mhe.nlmheAle   = nlmheAlessandri(sim.config.Ne,sim.mheAle.beta,sim.dynamic.F,sim.dynamic.h,...
...
%     setC(sim.mheLoss.nlmheLoss,2*sim.mheLoss.l_x);
    ...
    boxConst    = [];
    sim.mheLoss.nlmheLoss  = mheOptiLoss(sim.config.Ne,sim.dynamic.F,sim.dynamic.h,dims,boxConst,sim.config.Ts);
...
function sim = gen_state_trajectory(sim)    
    ...
    % plt
    xsim1                   = zeros(2,length(sim.data.xsim1)*sim.config.numSensors*floor(100/sim.config.fracTs));    
    xsim1(:,sim.config.numSensors*floor(100/sim.config.fracTs):sim.config.numSensors*floor(100/sim.config.fracTs):end)    = sim.data.xsim1;    

    xsim1Extra              = zeros(2,length(sim.data.xsim1Extra)*floor(100/sim.config.fracTs));    
    for i=sim.config.numSensors*floor(100/sim.config.fracTs):sim.config.numSensors*floor(100/sim.config.fracTs):length(xsim1Extra)
        xsim1Extra(:,i-(sim.config.numSensors-1):i) = sim.data.xsim1Extra(:,i/floor(100/sim.config.fracTs)-(sim.config.numSensors-1):i/floor(100/sim.config.fracTs));
    end

    ysim1Extra                   = zeros(2,length(sim.data.ysim1)*sim.config.numSensors*floor(100/sim.config.fracTs));
    ysim1ExtraAux = reshape(sim.data.ysim1,2,sim.config.numSensors*length(sim.data.ysim1));
    for i=sim.config.numSensors*floor(100/sim.config.fracTs):sim.config.numSensors*floor(100/sim.config.fracTs):length(ysim1Extra)
        ysim1Extra(:,i-(sim.config.numSensors-1):i) = ysim1ExtraAux(:,i/floor(100/sim.config.fracTs)-(sim.config.numSensors-1):i/floor(100/sim.config.fracTs));
    end
